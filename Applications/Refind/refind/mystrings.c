...
VOID MergeStrings(IN OUT CHAR16 **First, IN CHAR16 *Second, CHAR16 AddChar) {
    ...
    if (NewString != NULL) {
        ...
        NewString[0] = L'\0';
        if (*First != NULL) {
            StrCatS(NewString, Length1 + 1, *First);
            if (AddChar) {
        ...
        if (Second != NULL)
            StrCatS(NewString, Length1 + Length2 + 2,Second);
        MyFreePool(*First);
        *First = NewString;
    } else {
...
BOOLEAN LimitStringLength(CHAR16 *TheString, UINTN Limit) {
    ...
    while (SubString != NULL) {
        ...
        if (i >= StrLen(SubString)) {
            ...
            if (TempString != NULL) {
                StrCpyS(&SubString[1], sizeof(SubString), TempString);
...
BOOLEAN DeleteItemFromCsvList(CHAR16 *ToDelete, CHAR16 *List) {
    ...
    if ((Found = MyStrStr(List, ToDelete)) != NULL) {
        if ((Comma = MyStrStr(Found, L",")) == NULL) {
            ...
            StrCpyS(Found, sizeof(Found), &Comma[1]);
        } // if/else
        return TRUE;
    } else {
        return FALSE;
...
BOOLEAN ReplaceSubstring(IN OUT CHAR16 **MainString, IN CHAR16 *SearchString, IN CHAR16 *ReplString) {
    ...
    if (FoundSearchString) {
        ...
        if (NewString) {
            ...
            StrCpyS(NewString, sizeof(NewString), *MainString);
