...
def encode_plane(rawdata, identname, planename):
    ...
    print("  plane %s: compressed %d to %d (%.1f%%)" %
          (planename, rawlen, complen, float(complen) / float(rawlen) * 100.0))
...
for filename in sys.argv[1:]:
    ...
    if len(planes[0]) > 0 and planes[0] == planes[1] and planes[0] == planes[2]:
        ...
        (output_part, planeinfo[0]) = encode_plane(
            planes[0], identname, planenames[4])
        ...
        (output_part, planeinfo[0]) = encode_plane(
            planes[0], identname, planenames[0])
        output = output + output_part
        ...
        if planes[1] == planes[0]:
            ...
            (output_part, planeinfo[1]) = encode_plane(
                planes[1], identname, planenames[1])
            output = output + output_part
        ...
        if planes[2] == planes[0]:
            ...
            (output_part, planeinfo[2]) = encode_plane(
                planes[2], identname, planenames[2])
            output = output + output_part
    ...
    if len(planes[3]) > 0:
        if reduce(lambda x, y: x+y, planes[3]) == 0:
            ...
            (output_part, planeinfo[3]) = encode_plane(
                planes[3], identname, planenames[3])
            output = output + output_part

    output = output + \
        "static EEI_IMAGE eei_%s = { %d, %d, NULL, {\n" % (
            identname, width, height)
    for i in range(0, 4):
        output = output + "    { %s },\n" % planeinfo[i]
    output = output + "} };\n"

    f = FileIO("eei_%s.h" % identname, "w")
